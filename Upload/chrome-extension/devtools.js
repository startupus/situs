...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Handle reconnect request from panel
  if (message.type === "RECONNECT_WEBSOCKET") {
    console.log("Received reconnect request from panel");
    setupWebSocket();
  }
...
chrome.devtools.panels.create("BrowserToolsMCP", "icon48.png", "panel.html", (panel) => {
        ...
        if (message.type === "heartbeat-response") {
          ...
          chrome.tabs.captureVisibleTab(null, { format: "png" }, (dataUrl) => {
            if (chrome.runtime.lastError) {
              ...
              // Try alternative method using chrome.tabs.query
              chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
                if (tabs && tabs[0]) {
                  chrome.tabs.captureVisibleTab(tabs[0].windowId, { format: "png" }, (dataUrl2) => {
                    if (chrome.runtime.lastError) {
                      console.error("Alternative screenshot method also failed:", chrome.runtime.lastError);
                      ws.send(
                        JSON.stringify({
                      ...
                      console.log("Chrome Extension: Screenshot captured successfully (alternative method)");
                      sendScreenshotResponse(dataUrl2, message.requestId);
                    }
                  });
                } else {
                  ws.send(
            ...
            sendScreenshotResponse(dataUrl, message.requestId);
          });
...
// Function to send screenshot response
function sendScreenshotResponse(dataUrl, requestId) {
  const response = {
