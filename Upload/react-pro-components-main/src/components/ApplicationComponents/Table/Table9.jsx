    ...
    <thead>
      <tr className="bg-[#F6F8FB] dark:bg-dark-3">
        {headers.map((header, index) => (
          <th
            className={`px-4 py-6 first:pl-10 last:pr-10 last:text-right ${header.styles}`}
            key={index}
          >
    ...
    <tbody>
      {data.map((row, index) => (
        <tr className="border-b border-stroke dark:border-dark-3" key={index}>
          ...
          <td className="px-4 py-5">
            {row.status ? (
              <span className="inline-flex h-8 items-center justify-center rounded-sm bg-green px-5 text-base text-white">
                Approved
              </span>
            ) : (
