...
const AvatarItem = ({ img, size, bulletSize, active, border }) => {
  return (
    <div className={`h-${size} w-${size} relative rounded-full`}>
      <img
        src={img}
        alt="avatar"
      ...
      <span
        ...
        } absolute right-0 top-0 block h-${bulletSize} w-${bulletSize} rounded-full ${border} border-white dark:border-dark`}
      ></span>
    </div>
  );
};
