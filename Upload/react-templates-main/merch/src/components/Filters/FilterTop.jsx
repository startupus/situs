...
const FilterTop = () => {
  ...
  const toggleOptionSelect = (index, optionIndex) => {
    const updatedDropdownItems = [...dropdownItems];
    updatedDropdownItems[index].selectedOption =
      dropdownItems[index].options[optionIndex].value;
    setDropdownItems(updatedDropdownItems);
  };

  return (
    <>
      <div className="relative z-20 mb-10 rounded-lg border border-stroke bg-gray-1 py-5 pl-8 pr-5 dark:border-dark-3 dark:bg-dark-2">
        <div className="-mx-4 flex flex-wrap items-center justify-between">
            ...
            <div className="items-center sm:flex">
              {dropdownItems.map((item, index) => (
                <Dropdown
                  ...
                  toggleOptionSelect={(optionIndex) =>
                    toggleOptionSelect(index, optionIndex)
                  }
                  index={index}
                />
              ))}
          ...
          <div className="w-full px-4 md:w-3/12 lg:w-3/12 xl:w-4/12">
            <div className="flex items-center space-x-4 md:justify-end">
              <button className="flex h-10 w-10 items-center justify-center rounded-[5px] border border-primary bg-primary text-white hover:border-primary hover:bg-primary hover:text-white">
                <svg
              ...
              <button className="flex h-10 w-10 items-center justify-center rounded-[5px] border border-stroke bg-transparent text-dark hover:border-primary hover:bg-primary hover:text-white dark:border-dark-3 dark:text-white">
                <svg
                  className="fill-current"
                  width="20"
                  height="20"
