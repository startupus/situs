...
### 🚀 Проекты и продукты
- **Управление проектами**: создание, редактирование, архивирование
- **Типы проектов**: Website, Blog, Store
- **Продукты и категории**: полный цикл управления каталогом
- **Аналитика**: статистика по проектам и продажам
...
- **Система интеграций** (`src/api/services/integrations.api.ts`):
  - Каталог провайдеров (EMAIL_SMTP, N8N, WEBHOOK_GENERIC)
  - Управление инстансами по проектам
  - Health check с кешированием (60с) и rate limiting (10/мин)
  - Email preview и N8N workflow routing
- Проксирование `/api` через Vite (dev) на `http://localhost:3002`
- **SSE Real-time**:
  - `/api/projects/events` — события проектов (status/update/reorder)
  - `/api/realtime/integrations?projectId=xxx` — события интеграций с фильтрацией по проекту
  - Поддержка debouncing для массовых обновлений

### 🔌 Улучшения системы интеграций
- **UI/UX улучшения**:
  - ✅ Health индикаторы рядом со статусом (READY/ERROR/тестирование)
  - ✅ Кнопка "Проверить все" с параллельным тестированием
  - ✅ Toast-уведомления для всех операций (создание/обновление/тест)
  - ✅ Пагинация и сортировка списков интеграций
  - ✅ Help-поповеры с инструкциями для каждого провайдера
  - ✅ Маскирование секретных полей с показать/скрыть
- **Backend улучшения**:
  - ✅ Rate limiting на тестирование (10 запросов/мин на интеграцию)
  - ✅ Кеширование результатов health check (60 секунд)
  - ✅ Webhooks для аудита изменений интеграций
  - ✅ События integration_health_changed с сохранением в БД
  - ✅ Шифрование Integration.secrets (AES-256-GCM)
- **SSE улучшения**:
  - ✅ Фильтрация событий по проекту (?projectId=xxx)
  - ✅ Debouncing для массовых обновлений
- **Провайдеры**:
  - ✅ N8N: валидация routes map и UI-редактор маршрутов
  - ✅ EMAIL: предпросмотр шаблонов приглашений
- **Тестирование**:
  - ✅ E2E тесты health check для EMAIL/N8N
  - ✅ UI smoke тесты для фильтров и поиска
