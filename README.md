  ...
  - `components/` — компоненты приложения
    ...
    - `admin/` — административные компоненты
      - `menu/` — **Система управления меню** (модульная архитектура)
        - `MenuManager.tsx` — главный контейнер-оркестратор (~200 строк, упрощен)
        ...
        - `MenuDragDrop.tsx` — drag & drop с `dnd-kit-sortable-tree`
        - `MenuItemsList.tsx`, `MenuItemCard.tsx` — список и карточки пунктов
...
- **Стало**: 3 хука + 4 компонента, четкое разделение ответственности
