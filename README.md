...
### Текущее состояние (2025‑08‑09)
- Удалён временный Express‑сервер (`server/index.js`) — вся логика на NestJS
- Включён единый SSE‑эндпоинт на Nest (`/api/projects/events`)
- Список проектов и операции идут через Prisma (SQLite dev)
- Переключение статуса проекта (`PATCH /api/projects/:id/status`) обновляет БД и рассылает событие `project_status`
- DnD‑порядок карточек сохраняется в `Project.settings.orderIndex`
...
## 🧹 Депрекации/чистка
- Удалён `server/index.js` (Express мок) — не используется
