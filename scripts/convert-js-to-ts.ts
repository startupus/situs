...
function convertJSToTS(content: string, filePath: string): string {
  ...
  tsContent = tsContent.replace(/const\s+\{\s*([^}]+)\s*\}\s*=\s*require\(['"]([^'"]+)['"]\)/g, 'import { $1 } from \'$2\'');
  tsContent = tsContent.replace(/const\s+(\w+)\s*=\s*require\(['"]([^'"]+)['"]\)\.(\w+)/g, 'import { $3 as $1 } from \'$2\'');
...
function convertFile(jsFilePath: string): boolean {
  try {
    ...
    console.log(`✅ Конвертирован: ${path.relative(process.cwd(), jsFilePath)} -> ${path.relative(process.cwd(), tsFilePath)}`);
    return true;
  } catch (error) {
