...
const filesToFix = [
  {
    ...
    fixes: [
      { pattern: /const \[pageValues\] = usePageValues\(\) as \[\{\}, \(\) => void\]/g, replacement: 'const pageValues = usePageValues() as any' },
  ...
  },
  {
    ...
    fixes: [
      { pattern: /if \(isAdmin && !previewMode\) {/g, replacement: 'if (isAdmin) {' },
    ]
  },
  {
    file: 'src/components/redaktus/config/bricks/custom/Pokemon.tsx',
    fixes: [
      { pattern: /previewImageUrl: `\/bricks-preview-images\/pokemon\.png`,/g, replacement: '// previewImageUrl: `/bricks-preview-images/pokemon.png`,' },
    ]
  },
  {
    file: 'src/components/redaktus/config/NextLink.tsx',
    fixes: [
      { pattern: /<Link href=\{href\} target=\{target\} rel=\{rel\} className=\{anchorClassName\}>/g, replacement: '<Link href={href} className={anchorClassName}>' },
    ]
  },
  {
    file: 'src/components/redaktus/config/pageTypes.ts',
    fixes: [
      { pattern: /isEntity: true,/g, replacement: '// isEntity: true,' },
    ]
  },
  {
