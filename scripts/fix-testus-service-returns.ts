  ...
  private async fixAgentsFile(): Promise<void> {
    ...
    if (fs.existsSync(file)) {
      try {
        ...
        content = content.replace(/(\s+)res\.status\(400\)\.json\(/g, '$1return res.status(400).json(');

        content = content.replace(/(\s+)res\.status\(200\)\.json\(/g, '$1return res.status(200).json(');

        content = content.replace(/(\s+)res\.status\(500\)\.json\(/g, '$1return res.status(500).json(');
  ...
  private async fixAuthFile(): Promise<void> {
    ...
    if (fs.existsSync(file)) {
      try {
        ...
        content = content.replace(/(\s+)res\.status\(401\)\.json\(/g, '$1return res.status(401).json(');

        content = content.replace(/(\s+)res\.status\(403\)\.json\(/g, '$1return res.status(403).json(');

        // Добавляем return в конце middleware функций
        content = content.replace(/(\s+)next\(\);(\s+)}/g, '$1return next();$2}');
