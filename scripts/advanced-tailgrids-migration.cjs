...
const enhanceForRedactus = (content, fileName, category) => {
  ...
  if (!enhanced.includes('interface') && !enhanced.includes('Props')) {
    enhanced = enhanced.replace(/(import.*\n\n)/, `$1${propsInterface}\n\n`);
  }
  ...
  enhanced = enhanced
    ...
    // Параграфы
    .replace(/<p([^>]*?)>([^<{]*?[^}])<\/p>/g, '<p$1 contentEditable suppressContentEditableWarning={true}>$2</p>')
    // Спаны с текстом
    .replace(
      /<span([^>]*?)>([^<{]*?[^}])<\/span>/g,
...
async function advancedMigration() {
  ...
  for (const [categoryName, config] of Object.entries(MIGRATION_CONFIG)) {
    ...
    const components = findComponentsInCategory(categoryPath, config.priority, config.maxComponents);
...
const createIndexFiles = (migratedByCategory) => {
  Object.entries(migratedByCategory).forEach(([category, components]) => {
    ...
    const exports = components.map((comp) => `export { default as ${comp} } from './${comp}';`).join('\n');

    const typeExports = components.map((comp) => `export type { ${comp}Props } from './${comp}';`).join('\n');
