...
const updateIndexFiles = (migratedByCategory) => {
  Object.entries(BULK_MIGRATION_CONFIG.targetDirs).forEach(([category, dir]) => {
    ...
    const newExports = components.map((comp) => `export { default as ${comp} } from './${comp}';`).join('\n');

    const newTypeExports = components.map((comp) => `export type { ${comp}Props } from './${comp}';`).join('\n');
