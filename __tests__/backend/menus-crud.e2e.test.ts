  ...
  describe('MenuItems Special Endpoints', () => {
    ...
    it('tests authorized items filtering', async () => {
      ...
      const authorized = await fetch(
        `${API_BASE}/menu-items/authorized?menuTypeId=${menuTypeId}&accessLevels=PUBLIC,REGISTERED`,
      );
      ...
      if (authorized.status === 200) {
        ...
        expect(items.every((item: any) => ['PUBLIC', 'REGISTERED'].includes(item.accessLevel))).toBe(true);
      }
    });
    ...
    it('tests multi-parameter filtering (Joomla style)', async () => {
      ...
      const filtered = await fetch(
        `${API_BASE}/menu-items/items-by-filters?menuTypeId=${menuTypeId}&properties=level,component&values=1,Website`,
      );
      ...
      if (filtered.status === 200) {
        ...
        expect(items.every((item: any) => item.level === 1 && item.component === 'Website')).toBe(true);
      }
    });
  });
