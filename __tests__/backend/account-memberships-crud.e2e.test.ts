...
describe('AccountMemberships CRUD', () => {
  it('create, update role, delete membership', async () => {
    ...
    const ownerId: string | null = [200, 201].includes(regOwner.status)
      ? regOwner.json?.user?.id || null
      : 'test-user-id';
    const tokenOwner: string | null = [200, 201].includes(regOwner.status)
      ? regOwner.json?.tokens?.accessToken || null
      : null;
    ...
    const memberId: string | null = [200, 201].includes(regMember.status)
      ? regMember.json?.user?.id || null
      : 'test-user-id';
