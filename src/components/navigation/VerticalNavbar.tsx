...
const VerticalNavbar: React.FC<VerticalNavbarProps> = ({ logo, items, userProfile, className, onItemClick }) => {
  return (
          ...
          <nav>
            <ul>
              {items.map((item, index) => (
                <NavItemComponent key={item.id} item={item} onClick={() => onItemClick?.(item)} />
              ))}
            </ul>
          </nav>
        </div>
        ...
        {userProfile && (
          <div className="p-10">
            <div className="flex items-center">
              ...
              <div>
                <h6 className="text-base font-medium text-body-color">{userProfile.name}</h6>
  ...
  return (
    <li className="relative">
      ...
      {item.submenu && dropdownOpen && (
        <ul ref={dropdown} className="absolute left-0 top-full z-40 w-full bg-white shadow-lg">
