...
const SitusProfileSettings: React.FC = () => {
  ...
  const activeTab = useMemo(() => (['profile','general','notifications','security'].includes(rawTab) ? rawTab : 'profile'), [rawTab]);
    ...
    (async () => {
      try {
        ...
        const nextState = {
          ...
          avatar: data?.avatar || profileJson.avatar || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(fullName || 'User') + '&background=3b82f6&color=fff',
