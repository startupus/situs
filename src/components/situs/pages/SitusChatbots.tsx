...
const ProjectsSidebar: React.FC = () => {
  ...
  return (
    <div className="w-64 bg-white dark:bg-dark-2 border-r border-stroke dark:border-dark-3 p-4">
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-dark dark:text-white mb-2">Разделы проектов</h2>
        <p className="text-sm text-body-color dark:text-dark-6">Выберите тип проекта для просмотра</p>
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-stroke dark:border-dark-3">
        <div className="text-sm text-body-color dark:text-dark-6 mb-3">Быстрые действия</div>
        <Link
          to="/projects/new"
...
const SitusChatbots: React.FC = () => {
  ...
  const filteredChatbots = chatbots.filter((chatbot) => chatbot.name.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="flex">
      ...
      {/* Правая панель с чатботами */}
      <div className="flex-1 p-6">
        {/* Заголовок */}
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-semibold text-dark dark:text-white">Чатботы</h1>
              <p className="text-body-color dark:text-dark-6 mt-2">Управление чат-ботами и автоматизацией</p>
            </div>
            <Link
              to="/projects/new?type=chatbot"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="flex-shrink-0">
                      <svg width="24" height="24" viewBox="0 0 24 24" className="fill-current text-purple-500">
                        ...
                        <path d="M12 17C10.8954 17 10 16.1046 10 15H14C14 16.1046 13.1046 17 12 17Z" />
                      </svg>
                    </div>
                    <div>
                      <h3 className="font-semibold text-dark dark:text-white text-sm">{chatbot.name}</h3>
                      <p className="text-xs text-body-color dark:text-dark-6">{chatbot.platform}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                  <div className="text-center">
                    <div className="text-lg font-semibold text-dark dark:text-white">
                      {chatbot.visitors.toLocaleString()}
                    </div>
                    <div className="text-xs text-body-color dark:text-dark-6">Пользователи</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold text-dark dark:text-white">{chatbot.messages}</div>
                    <div className="text-xs text-body-color dark:text-dark-6">Сообщения</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold text-dark dark:text-white">{chatbot.orders}</div>
            ...
            {filteredChatbots.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold text-dark dark:text-white mb-2">Чат-боты не найдены</h3>
                <p className="text-body-color dark:text-dark-6 mb-4">
                  {searchTerm ? 'Попробуйте изменить поисковый запрос' : 'Создайте свой первый чат-бот'}
                </p>
                {!searchTerm && (
                  <Link
                    to="/projects/new?type=chatbot"
