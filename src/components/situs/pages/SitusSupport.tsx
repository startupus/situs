    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-dark dark:text-white">Поддержка</h1>
            <p className="mt-2 text-body-color dark:text-dark-6">Управление обращениями в службу поддержки</p>
          </div>
      ...
      <div className="mb-6 rounded-lg bg-surface dark:bg-dark-2 p-6 shadow">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <div>
            <label className="mb-2 block text-sm font-medium text-dark dark:text-white">Поиск</label>
            <input
              type="text"
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-dark dark:text-white">Статус</label>
            <select
              value={statusFilter}
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-dark dark:text-white">Приоритет</label>
            <select
      ...
      <div className="rounded-lg bg-white shadow dark:bg-dark-2">
              ...
              {filteredTickets.map((ticket) => (
                <tr key={ticket.id} className="hover:bg-gray-50 dark:hover:bg-dark-3">
                  <td className="px-6 py-4">
                    <div>
                      <div className="flex items-center">
                        <p className="text-sm font-medium text-dark dark:text-white">{ticket.title}</p>
                        <span className="ml-2 text-xs text-body-color dark:text-dark-6">#{ticket.id}</span>
                      </div>
                      <p className="mt-1 text-sm text-body-color dark:text-dark-6 line-clamp-2">{ticket.description}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span
                  ...
                  <td className="px-6 py-4 text-sm text-body-color dark:text-dark-6">{ticket.responses}</td>
                  <td className="px-6 py-4 text-sm text-body-color dark:text-dark-6">{formatDate(ticket.updatedAt)}</td>
                  <td className="px-6 py-4 text-right text-sm font-medium">
                    <button className="text-primary hover:text-primary/80">Открыть</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {filteredTickets.length === 0 && (
          <div className="py-12 text-center">
            ...
            <h3 className="mt-2 text-sm font-medium text-dark dark:text-white">Обращения не найдены</h3>
