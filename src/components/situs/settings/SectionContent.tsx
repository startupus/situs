        ...
        <>
          ...
          <div className="grid gap-8 md:grid-cols-2">
            ...
            <div className="bg-white rounded-lg p-6 shadow-lg dark:bg-dark-2">
              <h3 className="text-xl font-semibold text-dark dark:text-white mb-6">Основные настройки</h3>

              {/* Включение/отключение раздела */}
              <div className="mb-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-dark dark:text-white">Включить раздел</h4>
                      ...
                      <div className="space-y-2">
                        {value.map((item, index) => (
                          <div key={index} className="flex items-center space-x-2">
                            <span className="px-3 py-1 bg-primary/10 text-primary rounded-full text-sm">{item}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
            ...
            {/* Права доступа */}
            <div className="bg-white rounded-lg p-6 shadow-lg dark:bg-dark-2">
              <h3 className="text-xl font-semibold text-dark dark:text-white mb-6">Права доступа</h3>
              <div className="space-y-4">
                {section.permissions.map((permission: string) => (
                  <div key={permission} className="flex items-center justify-between">
                    <div>
                      <h4 className="font-medium text-dark dark:text-white">{permission}</h4>
                      <p className="text-sm text-body-color dark:text-dark-6">Разрешение на {permission}</p>
                    </div>
                    <label className="relative inline-flex cursor-pointer items-center">
