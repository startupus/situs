...
} from '../../../../api/services/integrations.api';
    ...
    <div className="p-6">
      <div className="flex items-center justify-between gap-3 border-b border-gray-200 dark:border-gray-700 mb-6">
        <div className="flex items-center gap-3">
          <button
            className={`py-2 px-3 ${tab === 'catalog' ? 'border-b-2 border-primary text-primary' : 'text-gray-600 dark:text-gray-300'}`}
            onClick={() => setTab('catalog')}
          >
            Каталог
          </button>
          <button
            className={`py-2 px-3 ${tab === 'installed' ? 'border-b-2 border-primary text-primary' : 'text-gray-600 dark:text-gray-300'}`}
      ...
      {tab === 'catalog' && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filteredProviders.map((p) => (
            <div
              key={p.key}
              className="rounded border border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-gray-900"
            >
              ...
              <div className="mt-3">
                <button
                  disabled={busy}
                  onClick={() => createInstance(p)}
                  className="inline-flex items-center px-3 py-2 text-sm rounded bg-primary text-white disabled:opacity-60"
                >
                  Подключить
                </button>
              </div>
      ...
      {tab === 'installed' && (
        <div className="space-y-3">
          ...
          {filteredInstances.map((inst) => (
            <div
              key={inst.id}
              className="rounded border border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-gray-900 flex items-center justify-between"
            >
              <div className="min-w-0">
                <div className="font-medium truncate flex items-center gap-2">
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold ${inst.status === 'READY' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700'} ${inst.status === 'ERROR' ? 'bg-red-100 text-red-800' : ''}`}
                  ...
                  {inst.isActive ? (
                    <span
                      className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-blue-100 text-blue-800"
                    ...
                    <span
                      className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold bg-gray-100 text-gray-700"
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {inst.isActive ? 'Выключить' : 'Включить'}
                </button>
              </div>
            </div>
          ))}
      ...
      {tab === 'edit' && editing && (
        <div className="max-w-2xl">
          <div className="mb-4">
            ...
            <div className="text-xs text-gray-500">
              [{editing.provider}/{editing.instanceKey}]
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                value={form.instanceKey || ''}
                onChange={(e) => setForm((f) => ({ ...f, instanceKey: e.target.value }))}
                className="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-3 py-2"
              />
            </div>
              ...
              <>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Ключ хранится в конфиге инстанса; отображение маскируется.
                  </p>
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({
                        ...
                        config: {
                          ...(f.config || {}),
                          security: { ...(f.config?.security || {}), hmacSecret: e.target.value },
                        },
                      }))
                    }
                    className="w-full rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-3 py-2"
                ...
                <div>
                  ...
                  {workflows && workflows.length === 0 && (
                    <div className="text-xs text-gray-500">Список пуст или нет доступа. Проверьте API Key.</div>
                  )}
                  {workflows && workflows.length > 0 && (
                  ...
                  <div className="mt-2">
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      Можно указать ID вручную, если список воркфлоу недоступен.
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-4">
                  <div className="mb-2">
                    ...
                    <p className="text-xs text-gray-500">
                      Свяжите ключ действия с конкретным воркфлоу n8n. Примеры ключей: invitations.send, email.send,
                      content.ai.process
                    </p>
                  </div>
