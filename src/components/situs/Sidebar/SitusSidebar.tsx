...
import { FiGrid, FiFolder, FiUsers, FiSettings, FiMenu as FiMenuIcon, FiArrowLeft } from 'react-icons/fi';
...
const SitusSidebar: React.FC<SitusSidebarProps> = ({ sidebarOpen, setSidebarOpen, isOpen, onClose }) => {
  ...
  const resolveIconNode = useCallback((item?: { icon?: string; iconLibrary?: string }, fallback?: React.ReactNode) => {
    ...
    if (name) {
      const lib = (
        item?.iconLibrary ||
        (name.startsWith('Fi') ? 'fi' : name.startsWith('Fa') ? 'fa' : name.startsWith('Md') ? 'md' : 'fi')
      ).toLowerCase();
  ...
  useEffect(() => {
    ...
    es.onmessage = async (ev) => {
      try {
        ...
        if (!t) return;
        try {
          console.debug('[SIDEBAR_SSE] event:', t);
        } catch {}
        if (t.startsWith('menu_')) {
          try {
            ...
            if (j?.success && Array.isArray(j.data)) {
              ...
              if (!j.data.length) {
                try {
                  console.warn('[SIDEBAR] admin-sidebar is empty');
                } catch {}
              }
            } else {
              try {
                console.warn('[SIDEBAR] failed to load admin-sidebar');
              } catch {}
            }
          } catch {}
          try {
            ...
            if (jp?.success && Array.isArray(jp.data)) {
              ...
              if (!jp.data.length) {
                try {
                  console.warn('[SIDEBAR] system-project-sidebar is empty');
                } catch {}
              }
            } else {
              try {
                console.warn('[SIDEBAR] failed to load system-project-sidebar');
              } catch {}
            }
          } catch {}
        }
  ...
  const projectMappedItems: AdminItem[] = useMemo(() => {
    ...
    if (systemProjectItems && systemProjectItems.length) {
      try {
        return mapSystemProjectToProject(systemProjectItems, projectId);
      } catch {}
    }
    // Fallback: маппинг admin-sidebar
    if (adminItems && adminItems.length) {
      try {
        return mapAdminToProject(adminItems, projectId);
      } catch {}
    }
    return projectItems || [];
          ...
          <div className="px-6 pt-6 pb-6">
            {projectId ? (
              <Link
                to={backTarget}
                title="Назад"
                className="h-8 w-8 rounded flex items-center justify-center border border-stroke dark:border-dark-3 text-dark dark:text-white hover:bg-gray-100 dark:hover:bg-dark-3"
              >
