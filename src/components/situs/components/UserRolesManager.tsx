...
const UserRolesManager: React.FC<UserRolesManagerProps> = ({
  ...
  const globalRoles = [
    { value: 'SUPER_ADMIN', label: 'Супер администратор', description: 'Полный доступ ко всем функциям системы', level: 100 },
    { value: 'STAFF', label: 'Персонал', description: 'Системные администраторы и поддержка', level: 80 },
    { value: 'AGENCY', label: 'Агентство', description: 'Управление собственными проектами и проектами клиентов', level: 60 },
            ...
            <>
              ...
              {activeTab === 'projects' && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {roles.filter(r => r.type === 'project').map(role => (
                      <div
                        key={role.id}
              ...
              {activeTab === 'accounts' && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {roles.filter(r => r.type === 'account').map(role => (
                      <div
                        key={role.id}
                        className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg"
                      >
