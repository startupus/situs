...
interface Filter4Props {
  title: string;
  children: string;
}

const Filter4: React.FC<Filter4Props> = () => {
  return (
...
const SidebarCard = ({ title, children }) => {
  ...
  return (
    <div className="redaktus-component" data-component-type="filter4">
    <div className="mb-9">
      <button
        onClick={() => {
          setIsOpen(!isOpen);
      ...
      <div className={`${isOpen ? "block" : "hidden"} space-y-4 pt-6`}>
        {children}
      </div>
    </div>
  );
