  ...
  return (
    <div
      className={`relative mb-[30px] overflow-hidden ${
        big ? 'min-h-[400px] md:min-h-[450px] lg:min-h-[570px]' : 'min-h-[270px]'
      }`}
    >
      <img src={img} alt="product" className="absolute h-full w-full object-cover object-center" />
      <div className="absolute bottom-0 left-0 flex h-3/4 w-full items-end bg-linear-to-b from-[#212B3600] to-[#212B36DA] p-6">
        <div>
          <h3>
            <a href={link} className="mb-2 block text-lg font-semibold text-white hover:text-primary xl:text-xl">
              {title}
            </a>
          </h3>
          <div className="flex flex-wrap items-center">
            <span className="mt-0.5 inline-block pr-[34px] text-lg font-medium leading-none text-white">{price}</span>
            <div className="flex items-center space-x-1">
              ...
              <Star />
              <Star />
              <span className="pl-1 text-base font-medium text-white">{rating}</span>
            </div>
          </div>
        </div>
      </div>
...
const Star = () => {
  return (
    <svg className="h-4 w-4" viewBox="0 0 17 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
