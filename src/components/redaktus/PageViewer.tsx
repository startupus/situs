...
const blockRegistry: Record<string, React.ComponentType<any>> = {
  ...
  'container-block': () => (
    <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded">Container Block (Coming Soon)</div>
  ),
  'columns-block': () => <div className="p-4 bg-gray-100 dark:bg-gray-800 rounded">Columns Block (Coming Soon)</div>,
};

const RedaktusPageViewer: React.FC<RedaktusPageViewerProps> = ({
  page,
  main = false,
  className = '',
  onBlockUpdate,
    ...
    return (
      <div key={brick.id || index} className="relative group">
        ...
        <div className="absolute top-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
          <div className="bg-blue-500 text-white text-xs px-2 py-1 rounded">{brick.type}</div>
        </div>
        ...
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-10 flex space-x-1">
          <button className="bg-gray-500 text-white text-xs px-2 py-1 rounded hover:bg-gray-600">⬆️</button>
          <button className="bg-gray-500 text-white text-xs px-2 py-1 rounded hover:bg-gray-600">⬇️</button>
          <button
        ...
        <div className="block-container border-0">
          <div className="site-content">
            <BlockComponent {...brick.props} onUpdate={handleBlockUpdate} />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={className}>
      {page.content.length === 0 ? (
        ...
        <div className="space-y-0">{page.content.map((brick: any, index: number) => renderBlock(brick, index))}</div>
      )}
    </div>
  );
};
