  ...
  return (
    <div className="w-full px-4 lg:w-5/12 2xl:w-1/3">
      <div className="w-full rounded-lg border border-stroke bg-white lg:w-auto">
        <h3 className="border-b border-stroke py-4 px-7 text-base font-medium text-black">{title}</h3>
        <div className="p-7">
          <div className="mb-4 flex items-center">
            <div className="mr-3 h-[55px] w-full max-w-[55px]">
              <img src={img} alt="photo" className="h-full w-full rounded-full object-cover object-center" />
            </div>
            <div>
              <p className="mb-[6px] text-base text-black">{subtitle}</p>
              <div className="flex space-x-[10px]">
                <button className="text-sm text-body-color hover:text-danger">Delete</button>
                <label htmlFor="upload" className="cursor-pointer text-sm text-primary">
                  Update
                  <input type="file" className="sr-only" name="upload" id="upload" />
                </label>
              </div>
            </div>
          </div>
          <div className="flex min-h-[180px] w-full items-center justify-center rounded-md border border-dashed border-primary bg-gray p-3">
            <div className="text-center">
              ...
              <p className="mx-auto max-w-[126px] text-xs text-body-color">SVG, PNG, JPG or GIF (max, 800 X 800px)</p>
            </div>
          </div>
        </div>
      </div>
