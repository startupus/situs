...
export class ProjectPagesController {
  ...
  @Scopes('PROJECT_READ')
  async list(@Param('projectId') projectId: string, @Query('page') page?: string, @Query('limit') limit?: string) {
  ...
  async reorder(@Param('projectId') projectId: string, @Body() body: ReorderPagesDto) {
    ...
    const pages = await this.prisma.page.findMany({
      where: { id: { in: body.ids }, product: { projectId } },
      select: { id: true },
    });
    const valid = new Set(pages.map((p) => p.id));
    const ordered = body.ids.filter((id) => valid.has(id));
    await this.prisma.$transaction(
      ordered.map((id, idx) => this.prisma.page.update({ where: { id }, data: { orderIndex: idx } })),
    );
