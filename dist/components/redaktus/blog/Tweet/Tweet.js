...
const Tweet = ({ id, placeholder, align, cards, conversation, theme, }) => {
    const twitterEmbedRef = useRef(null);
    const [isLoading, setIsLoading] = useState(true);
    const { isDarkColorMode } = useContext(React.createContext({ isDarkColorMode: false }));
    useEffect(() => {
        const isBlackTheme = theme === 'dark' || (theme === 'auto' && !!isDarkColorMode);
        const twTheme = isBlackTheme ? 'dark' : '';
    ...
    return (_jsx(Section, { children: _jsx(Container, { children: _jsx("div", { ref: twitterEmbedRef, children: isLoading && placeholder }) }) }));
};
Tweet.schema = {
