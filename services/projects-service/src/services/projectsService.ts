...
// Инициализируем Prisma Client для проектов
...
export class ProjectsService {
  ...
  static async getProjects(ownerId: string, filters: ProjectFilters = {}) {
    try {
      const {
        search,
        status,
      ...
      // Выполняем запрос с подсчетом общего количества
        ...
        prisma.project.findMany({
          ...
          include: {
            pages: {
              ...
              take: 5 // Ограничиваем количество страниц в ответе
            },
            _count: {
              select: { pages: true }
            }
  ...
  static async getProject(projectId: string, ownerId?: string) {
    try {
  ...
  static async createProject(data: CreateProjectData) {
    try {
      ...
      // Генерируем situs домен если он указан
      let situsUrl = null;
      if (data.domain) {
  ...
  static async updateProject(projectId: string, ownerId: string, data: UpdateProjectData) {
    try {
  ...
  static async publishProject(projectId: string, ownerId: string) {
    try {
      ...
      // Получаем обновленный проект для логирования
